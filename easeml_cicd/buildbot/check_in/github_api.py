...
class GithubClient:
    def __init__(
        ...
        repo_slug=None,is_action=False, user_agent_prefix=None,
        ...
        if not is_action:
            ...
            self._gh_client,self.auth_token = get_installation_client(
                self._gh_int, installation_id,
                github_url,
            )
        else:
            self._gh_client, self.auth_token = github.Github(
                installation_id,
                base_url=github_url,
            ), installation_id
    ...
    def _get_check_lister(self, commit_ref):
        checker = self._get_check_caller()
        return partial(
            checker,
            url=f'/repos/{self._repo_slug}/commits/{commit_ref}/check-runs',
            verb='GET',
        )
    ...
    def list_checks(self, commit_ref):
        check_updater = self._get_check_lister(commit_ref)
        headers, data = check_updater()
        return data
...
def get_installation_client(
    ...
    auth_token=get_installation_auth_token(gh_integration, install_id)
